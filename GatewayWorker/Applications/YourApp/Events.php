...
class Events
{
    ...
    {
        self::$db = new \Workerman\MySQL\Connection('127.0.0.1', '3306', 'YU5zBmH9wFvxGkT', 'MSEb7KnGmFEpp6Gr', 'yu5zbmh9wfvxgkt');
        self::$name = self::$db->column('SELECT name FROM apoccdio_bullet_name');
        if ($worker->id == 0) {
            //定时任务
        ...
        if ($worker->id == 2) {
            //定时热度
            Timer::add(1, function () {
                $res = self::$db->query('SELECT room_number,heat FROM apoccdio_direct');
                foreach ($res as $item) {
                    $res = array(
                        'type' => 'heat',
                        'num' => $item['heat'],
        ...
        if ($worker->id == 3) {
           ...
           Timer::add(1, function () {
                //数据库链接
                $bullet = self::$db->single('SELECT text FROM apoccdio_direct_bullet ORDER BY RAND() LIMIT 1');
                $name = self::$name;
                $name = $name[array_rand($name)];
                $name != '游客' ? $level = mt_rand(3, 30) : $level = 1;
                $res = array(
