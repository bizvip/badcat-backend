...
class Dvideo extends Backend
{
    ...
    // 搜索字段
    protected $searchFields = 'vod_name';
    ...
    public function add()
    {
        ...
        for ($i = 0; $i < count($name); $i++) {
            $vod_class[$id[$i]] = $name[$i];
        }
        $this->assign('vod_class', $vod_class);
        ...
        // 所属专题
        $vod_belong = ['0'=>'选择所属'];
        $name = \app\admin\model\Belong::column('title');
        $id = \app\admin\model\Belong::column('id');
        for ($i = 0; $i < count($name); $i++) {
            $vod_belong[$id[$i]] = $name[$i];
        }
        $this->assign('vod_belong', $vod_belong);
        
        // 收费方式
        $vod_istoll = ['1'=>'免费','2'=>'会员'];
        $this->assign('vod_istoll', $vod_istoll);
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    // 随机添加点赞数（取值范围）
                    $params['vod_hits'] = mt_rand(30,199);
                    // 随机添加评论数（取值范围）
                    $params['vod_comments'] = mt_rand(1,19);
                    // 随机添加浏览量
                    // $params['browse'] = mt_rand(1087,67985);
                    $result = $this->model->allowField(true)->insertGetId($params);
                    if (isset(input()['vod_comments'])) {
                        $comment = input()['vod_comments'];
                        $size = count($comment);
                        for ($i = 0; $i < $size; $i++) {
