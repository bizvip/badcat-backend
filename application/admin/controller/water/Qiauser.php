 ...
 * @icon fa fa-circle-o
 */
class Qiauser extends Backend
{

    /**
     * Qiauser模型对象
     * @var \app\admin\model\water\Qiauser
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\water\Qiauser;
        $list = ['待审核', '已通过', '已拒绝'];
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $total = $this->model
                ...
                ->where($where)
                ->where('status', 0)  // 状态：0=APP真实用户 1=后台虚拟用户
                // 状态：待审核
                ->where('tong',0)
                ->order($sort, $order)
                ->count();
