 ...
 * @icon fa fa-comment
 */
class Commenta extends Backend
{

    /**
     * Comment模型对象
     * @var \app\admin\model\Commenta
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\water\Commenta;
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $total = $this->model
                ->where($where)
                ->with(['qia'])
                ->where('commenta.status',1)  // 状态：0=后台虚拟用户 1=APP真实用户
                ->order($sort, $order)
                ->count();
            ...
            foreach ($list as $row) {
                ...
                // $row->getRelation('qia')->visible(['title']);
            }
