...
class Detail extends Backend
{
	...
	// 自定义选择分类推荐
	public function fenleituijian($title, $housename, $houseimg, $img, $address, $roomurl, $xuhao)
	{
		$this->assign('title', $title);
		$this->assign('housename', $housename);
		$belong = [];
		$classname = \app\admin\model\Directclass::column('title');
		$classid = \app\admin\model\Directclass::column('id');
		for ($i = 0; $i < count($classname); $i++) {
			$belong[$classid[$i]] = $classname[$i];
		}
		$this->assign('belong', $belong);

		$class_id = input('classid'); //分类id
