...
class Tvideocomment extends Api
{
    ...
    public function tvideo(Request $request)
    {
        ...
        $req = $request->get();
        $count = Comments::where(['class' => $req['class'], 'video_id' => $req['id']])->count();
        if ($count < $req['num']) {
            $size = $req['num'] - $count;
            for ($i = 0; $i < $size; $i++) {
                ...
                $comment = db::table('apoccdio_text')->where(['class' => $req['class']])->orderRaw('rand()')->value('text');
                Comments::insert(['name' => $name, 'avator_image' => $photo, 'content' => $comment, 'video_id' => $req['id'], 'class' => $req['class'], 'creat_time' => date('Y-m_d H:i:s'),'level'=>mt_rand(0,2)]);
            }
        }
        $res = Comments::where(['class' => $req['class'], 'video_id' => $req['id'], 'tong' => 1])->page($req['current'], $req['every'])->order('creat_time desc')->select();
        $res ? $res->toArray() : '';
        $this->success('评论查询', $res, 200);
    }
...
//            }
    ...
    {
        $req = $this->request->post();
        $res = \app\admin\model\Dvideo::where('id', $req['id'])->setInc('browse', 1);
        if ($res) {
            $this->success('ok', '', 200);
        } else {
