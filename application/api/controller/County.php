...
class County extends Api
{
    ...
    //是否有邀请码
    public function allowinvitecode()
    {
        $res = Config::where('name', 'invite_code')->value('value');
        $this->result('全部取消', $res, 200);
    }
    //支付
    public function pay()
    ...
    //   $res = db::table('fsq_video')->where(['class'=>1])->order('create_time')->limit(60)->select();
    //   foreach($res as &$item){
    //         $all = suiji();
    ...
    //         db::table('fsq_community')->insert($params);
    //   }
    // }
    public function video(){
        $res = db::table('fsq_community')->where('comment',0)->limit(60)->select();
        foreach ($res as $value){
            db::table('fsq_community')->where('id',$value['id'])->update(['comment'=>mt_rand(1,5)]);
        }
    echo count($res);
