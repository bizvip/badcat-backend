...
(function ($) {
    ...
    window.FManager = function (options) {
        ...
        function createDialog() {
            ...
            if (options.top == 0) {
                options.top = (Math.max(0, $(window).height() - o.dialog.height())) / 2;
            }
            o.dialog.css({
            ...
            $(".wrapper", o.dialog).css("max-height", ($(window).height() - 100) + "px");
            //给对话框添加拖拽事件
        ...
        function appendFiles(data) {
            ...
            $.each(data, function (idx, item) {
                ...
                var oriExtension = getFileExt(item.oriURL);
                if (oriExtension == '') oriExtension = "default";
                oriExtension = oriExtension.toLowerCase();
                //如果不是图片，则根据文件的后缀名去加载对应的缩略图
                var imgSize = formatFileSize(item.filesize) + (item.filename ? " - " + item.filename : ''); //图片尺寸
                if ("mp4|mpeg|flv|mp3|wav|acc|webm".indexOf(oriExtension) > -1) {
                    imgSize = formatFileSize(item.filesize) + (item.filename ? " - " + item.filename : ''); //如果是文件则显示文件大小
                    builder.append('<span class="icon-placeholder" data-src="' + item.oriURL + '"><video width="113" height="113" src="' + item.oriURL + '" controls></video></span>');
                } else if ("jpg|jpeg|gif|png|bmp|webp|svg".indexOf(extension) == -1) {
                    imgSize = formatFileSize(item.filesize) + (item.filename ? " - " + item.filename : ''); //如果是文件则显示文件大小
                    builder.append('<span class="icon-placeholder icon-default icon-' + extension + '" data-src="' + item.oriURL + '"></span>');
                } else {
                    imgSize = item.width + 'x' + item.height + ' ' + imgSize;
